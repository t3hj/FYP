...
def render_hero(active_theme: str) -> None:
    with st.container():
        ...
        if st.button(
            ...
            if is_logged_in():
                # Logged in — open the quick upload form as normal
                st.session_state.show_quick_upload = True
                st.session_state.active_tab = "upload"
                st.rerun()
            else:
                # Not logged in — open the login modal immediately and
                # notify the user so they understand why
                st.session_state.ll_auth_modal = "login"
                st.session_state.ll_scroll_to_modal = True
                st.session_state._hero_auth_nudge = True
                st.rerun()

        # Toast notification if the user was redirected from the hero button.
        # st.toast renders in the corner regardless of scroll position.
        if st.session_state.pop("_hero_auth_nudge", False):
            st.toast(
        ...
        with col:
            st.markdown(
                ...
                <div class="ll-kpi-card">
                    ...
                    <div class="ll-kpi-hint">{escape(hint)}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )